...
ggplot() +
  ...
  geom_point(data = data.frame(MU[[1]]) %>% summarise_all(., mean), 
             mapping=aes(x=X1, y=X2), 
             size= 5, 
             color = 'green') +
  geom_point(data = data.frame(MU[[2]]) %>% summarise_all(., mean), 
             mapping=aes(x=X1, y=X2), 
             size= 5, 
             color = 'red') +
...
mm$yhat = compute_bayes_classifier(mm, centroids=MU, sigma=SIGMA^2, 
                                   omega=W, pi=PI, response='class')
mm$y = the_data$ytest
...
bayes_x_grid = compute_bayes_classifier(x_grid, 
                                        centroids=MU, 
                                        sigma=SIGMA^2, 
                                        omega=W, 
                                        pi=PI, 
