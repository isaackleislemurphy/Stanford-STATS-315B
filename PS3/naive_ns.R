...
lapply(1:nrow(grid_df), function(j){
  ...
  lapply(1:length(dev_data), function(i){
    ...
    result = fit_predict(
      ...
      formula=as.formula(paste0("response ~ county + ns(date_idx, df=", as.character(deg_free), ")"))
    ) 
    result %>%
...
# extract FV results
grid_results = extract_folds_outer(result_naive_ns, grid_df, yhat_cols = "yhat", collapse_func=score_loss) %>% 
  group_by(df) %>%
